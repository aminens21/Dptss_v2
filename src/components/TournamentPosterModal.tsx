...
type PosterTheme = 'orange_vibrant' | 'royal_blue' | 'morocco_emerald' | 'crimson_gold' | 'clean_slate';
...
export const TournamentPosterModal: React.FC<TournamentPosterModalProps> = ({
  ...
  const handlePrint = () => {
    if (!posterRef.current) return;
    const printContent = posterRef.current.outerHTML;
    ...
    const fontDef = POSTER_FONTS.find(f => f.id === selectedFont) || POSTER_FONTS[0];
    ...
    printWindow.document.write(`
      ...
      <html dir="rtl" lang="ar">
        <head>
          <meta charset="utf-8" />
          <title>ملصق ${championshipTitle}</title>
          ...
          <style>
            ...
            body {
              margin: 0;
              padding: 0;
              font-family: ${fontDef.css}, 'Cairo', sans-serif;
              background: #fff;
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-6xl max-h-[96vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-100/90 px-3 py-1.5 gap-2 shrink-0">
          <div className="flex items-center gap-1 bg-slate-200/80 p-0.5 rounded-xl border border-slate-300/70 w-full sm:w-auto">
            <button
              ...
              className={`flex-1 sm:flex-none px-3 py-1 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
                viewMode === 'split'
                  ? 'bg-white text-orange-700 shadow-xs border border-slate-200/80 font-black'
          ...
          <div className="hidden md:flex items-center gap-1.5 text-[11px] font-bold text-slate-600 bg-white px-2.5 py-1 rounded-lg border border-slate-200 shadow-3xs">
            ...
            <span>ورقة A4 عمودي (210 × 297 مم)</span>
          </div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-y-auto lg:overflow-hidden">
          ...
          <div className={`border-l border-slate-200 overflow-y-auto p-4 sm:p-5 space-y-4 bg-slate-50/70 order-2 lg:order-1 ${
            ...
            {/* Live Synchronized Banner on Mobile */}
            <div className="bg-orange-50 border border-orange-200/80 rounded-xl p-2.5 flex items-center justify-between text-xs text-orange-950 font-bold">
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-orange-600" />
                <span>التعديلات تنعكس فورياً ومباشرة على الملصق A4</span>
              </span>
              {viewMode === 'split' && (
                <button
                  type="button"
                  onClick={() => setViewMode('preview')}
                  className="text-orange-700 underline text-[11px] hover:text-orange-900 cursor-pointer"
                >
                  معاينة بحجم كامل
                </button>
              )}
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-2">
              <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                <span>السمة اللونية والتصميم</span>
              </label>
              <div className="grid grid-cols-5 gap-1.5">
                <button
                  ...
                  title="برتقالي رياضي نشط (مثل النموذج الرسمي)"
                >
                  🟠 برتقالي
                </button>
                <button
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-2">
              <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                <span>نوع خط الملصق الإعلاني (Font)</span>
              </label>
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  <span>التحكم في حجم الشعارات (الرأسية)</span>
                </label>
                <button
                  type="button"
                  onClick={() => {
            ...
            {/* Title Section (No category, no school level) */}
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-2">
              <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                <Trophy className="h-3.5 w-3.5 text-amber-600" />
                <span>عنوان البطولة الرئيسي (بارز في وسط الملصق)</span>
              </label>
              <input
                type="text"
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  <span>الجهة المنظمة والفرع الإقليمي</span>
                </label>
              </div>
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-2.5">
              <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                <span>موعد ومكان إجراء المنافسات</span>
              </label>
              <div className="space-y-2">
                <div>
            ...
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-3xs space-y-2">
              <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                <span>الرسم الظلي الرياضي أو صورة الخلفية</span>
              </label>
          ...
          <div className={`bg-slate-200/90 p-2 sm:p-5 overflow-y-auto overflow-x-hidden flex flex-col items-center justify-start lg:justify-center order-1 lg:order-2 flex-1 pb-24 sm:pb-8 ${
            ...
            <div className="w-full flex justify-center overflow-x-auto py-0.5">
              <div
                ...
                className={`w-full max-w-[340px] xs:max-w-[380px] sm:max-w-[440px] md:max-w-[480px] min-h-[500px] xs:min-h-[550px] sm:min-h-[620px] aspect-[1/1.414] rounded-xl sm:rounded-3xl shadow-2xl border-2 sm:border-4 ${currentTheme.accentBorder} ${currentTheme.bg} p-2 xs:p-3 sm:p-5 md:p-6 flex flex-col justify-between relative overflow-hidden select-none transition-transform`}
                style={{
              ...
              <div className="absolute top-0 right-0 w-20 sm:w-32 h-20 sm:h-32 bg-gradient-to-bl from-orange-400/10 to-transparent rounded-bl-full pointer-events-none z-0" />
              <div className="absolute top-0 left-0 w-20 sm:w-32 h-20 sm:h-32 bg-gradient-to-br from-blue-400/10 to-transparent rounded-br-full pointer-events-none z-0" />
              ...
              <div className="relative z-10 w-full flex items-center justify-between pb-1 sm:pb-2.5 border-b sm:border-b-2 border-slate-300/60">
                ...
                <div
                  ...
                  style={{ width: `${Math.max(Math.round(frmssLogoSize * 0.75) + 6, 42)}px` }}
                >
                  {officialLogos?.frmssLogo ? (
                    <img
                      src={officialLogos.frmssLogo}
                      alt="FRMSS"
                      style={{ height: `${Math.max(frmssLogoSize, 26)}px` }}
                      className="object-contain drop-shadow-xs transition-all max-h-6 sm:max-h-12"
                    />
                  ) : (
                    <div className="flex flex-col items-center">
                      <AppLogo size={Math.max(Math.round(frmssLogoSize * 0.7), 22)} />
                      <span className="text-[6px] sm:text-[8px] font-black text-slate-700 tracking-tighter mt-0.5">FRMSS</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center px-1 text-center">
                  {officialLogos?.ministryLogo ? (
                    <img
                      ...
                      alt="شعار الوزارة"
                      style={{ height: `${Math.max(ministryLogoSize, 26)}px` }}
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <svg
                        ...
                        className="mb-0.5 transition-all max-w-[24px] sm:max-w-[42px]"
                        viewBox="0 0 100 100"
                        fill="none"
                      ...
                      <span className="text-[7.5px] xs:text-[8.5px] sm:text-[11px] font-black text-slate-800 leading-tight">المملكة المغربية</span>
                      <span className="text-[6.5px] xs:text-[7.5px] sm:text-[10px] font-bold text-slate-700 leading-tight">وزارة التربية الوطنية والتعليم الأولي والرياضة</span>
                    </div>
                  )}
                </div>
              ...
              <div className="relative z-10 text-center my-2 sm:my-3.5 space-y-1">
                <div className={`text-[8.5px] xs:text-[10px] sm:text-sm font-black ${currentTheme.sloganPrefix}`}>
                  {organizerPrefix}
                </div>
                <div className={`text-[10px] xs:text-[12px] sm:text-base font-extrabold ${currentTheme.subHeaderColor} tracking-tight leading-snug`}>
                  {organizer}
                </div>
                <div className="text-[8px] xs:text-[9.5px] sm:text-xs font-black text-slate-600 pt-0.5">
                  {partnerPrefix}
                </div>
                <div className={`text-[9px] xs:text-[10.5px] sm:text-sm font-extrabold ${currentTheme.partnerColor} leading-tight`}>
                  {partner}
                </div>
              </div>

              {/* --- 3. Prominent Championship Title Badge --- */}
              <div className="relative z-10 my-2 sm:my-4 flex justify-center">
                <div className="relative w-full max-w-[98%]">
                  {/* Outer glow / border shape */}
                  <div className={`py-2.5 xs:py-3.5 sm:py-5 px-3 sm:px-6 rounded-2xl sm:rounded-3xl ${currentTheme.titleBg} text-center border sm:border-2 border-white/40 transform transition-transform shadow-md sm:shadow-xl`}>
                    <div className="text-[8px] xs:text-[10px] sm:text-xs font-extrabold tracking-wider opacity-90 mb-1">
                      منافسات الرياضة المدرسية
                    </div>
                    <div className="text-[12px] xs:text-[15px] sm:text-2xl font-black tracking-tight leading-tight drop-shadow-xs">
                      {championshipTitle}
                    </div>
                  </div>
                </div>
              ...
              {/* --- 4. Slogan Section --- */}
              <div className="relative z-10 text-center my-1.5 sm:my-3">
                <span className={`text-[7.5px] xs:text-[9px] sm:text-[11px] font-black ${currentTheme.sloganPrefix} uppercase tracking-widest block mb-1`}>
                  تحت شعار
                </span>
                <div className="inline-block bg-white/95 backdrop-blur-xs px-3 sm:px-5 py-1 sm:py-1.5 rounded-full border border-slate-300/80 shadow-2xs">
                  <span className="text-[9px] xs:text-[11px] sm:text-sm font-extrabold text-slate-800 tracking-tight">
                    "{slogan}"
                  </span>
                </div>
              </div>
              ...
              <div className="relative z-10 grid grid-cols-2 gap-1.5 sm:gap-3 my-2 sm:my-4">
                {/* Date & Time Badge */}
                <div className={`p-2 xs:p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl border ${currentTheme.badgeBg} flex items-center gap-2 sm:gap-3`}>
                  <div className={`w-6 h-6 xs:w-7.5 xs:h-7.5 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-orange-100 flex items-center justify-center ${currentTheme.iconColor} shrink-0`}>
                    <Calendar className="h-3.5 w-3.5 sm:h-5 sm:w-5" />
                  </div>
                  <div className="min-w-0">
                    <div className="text-[8px] xs:text-[9.5px] sm:text-xs font-black text-slate-900 leading-snug truncate">{dateText}</div>
                    <div className="text-[7px] xs:text-[8px] sm:text-[10px] font-extrabold text-orange-600 truncate mt-0.5">{timeText}</div>
                  </div>
                </div>

                {/* Location / Venue Badge */}
                <div className={`p-2 xs:p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl border ${currentTheme.badgeBg} flex items-center gap-2 sm:gap-3`}>
                  <div className={`w-6 h-6 xs:w-7.5 xs:h-7.5 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-red-100 flex items-center justify-center text-red-600 shrink-0`}>
                    <MapPin className="h-3.5 w-3.5 sm:h-5 sm:w-5" />
                  </div>
                  <div className="min-w-0">
                    <div className="text-[8px] xs:text-[9.5px] sm:text-xs font-black text-slate-900 leading-snug truncate">{venueText}</div>
                    <div className="text-[7px] xs:text-[8px] sm:text-[10px] font-extrabold text-slate-600 truncate mt-0.5">{resolvedDirName}</div>
                  </div>
                </div>
              </div>

              {/* --- 6. Dynamic Sport Silhouette & Track Graphic --- */}
              <div className="relative z-10 mt-auto pt-0.5 sm:pt-1.5 flex items-end justify-center min-h-[50px] xs:min-h-[65px] sm:min-h-[105px] overflow-hidden">
                {/* Athletic Finish line / Running Silhouette */}
                {sportVisual === 'athletics' && (
                  <div className="w-full relative flex flex-col items-center">
                    <svg viewBox="0 0 500 160" className="w-full h-14 xs:h-16 sm:h-26" fill="none" xmlns="http://www.w3.org/2000/svg">
                      {/* Running Track Lanes */}
                ...
                {sportVisual === 'football' && (
                  <div className="w-full relative flex items-center justify-between px-3 sm:px-6">
                    <svg viewBox="0 0 400 130" className="w-full h-14 xs:h-16 sm:h-26" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {sportVisual === 'basketball' && (
                  <div className="w-full flex items-center justify-center">
                    <svg viewBox="0 0 350 130" className="w-full h-14 xs:h-16 sm:h-26" fill="none" xmlns="http://www.w3.org/2000/svg">
                      {/* Hoop */}
                ...
                {(sportVisual === 'table_tennis' || sportVisual === 'chess' || sportVisual === 'trophy') && (
                  <div className="w-full flex items-center justify-center py-1">
                    <div className="flex items-center gap-3 sm:gap-6">
                      <div className="w-8 h-8 sm:w-14 sm:h-14 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center text-lg sm:text-3xl shadow-sm">
                        🏆
                      </div>
                      <div className="text-center">
                        <div className="text-[10px] sm:text-sm font-black text-slate-800">تنافس رياضي شريف</div>
                        <div className="text-[8px] sm:text-[10px] font-extrabold text-amber-700">قيم التعاون والتميز المدرسي</div>
                      </div>
                      <div className="w-8 h-8 sm:w-14 sm:h-14 rounded-xl bg-orange-500/15 border border-orange-500/30 flex items-center justify-center text-lg sm:text-3xl shadow-sm">
                        🥇
                      </div>
                    </div>
                  </div>
              ...
              {/* --- Bottom Footer Strip --- */}
              <div className="relative z-10 pt-1 sm:pt-2 border-t border-slate-300/60 text-center space-y-0.5">
                <div className="text-[7.5px] xs:text-[8.5px] sm:text-[10px] font-bold text-slate-700 leading-tight">
                  البوابة الرقمية لتدبير البطولات المدرسية الاقليمية والجهوية والوطنية
                </div>
                <div className="text-[6.5px] xs:text-[7.5px] sm:text-[9px] font-medium text-slate-500">
